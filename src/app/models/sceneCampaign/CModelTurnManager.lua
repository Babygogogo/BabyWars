...
local CModelTurnManager = requireBW("src.global.functions.class")("CModelTurnManager")
...
local function getNextTurnAndPlayerIndex(self, playerManager)
    ...
    while (true) do
        ...
        assert(nextPlayerIndex ~= self.m_PlayerIndex, "CModelTurnManager-getNextTurnAndPlayerIndex() the number of alive players is less than 2.")
...
local function resetVisionOnClient(self)
    assert(not IS_SERVER, "CModelTurnManager-resetVisionOnClient() this shouldn't be called on the server.")
...
--------------------------------------------------------------------------------
function CModelTurnManager:ctor(param)
...
--------------------------------------------------------------------------------
function CModelTurnManager:toSerializableTable()
    return {
...
function CModelTurnManager:toSerializableTableForPlayerIndex(playerIndex)
    return self:toSerializableTable()
end

function CModelTurnManager:toSerializableReplayData()
...
--------------------------------------------------------------------------------
function CModelTurnManager:onStartRunning(modelSceneWar)
...
--------------------------------------------------------------------------------
function CModelTurnManager:getTurnIndex()
    return self.m_TurnIndex
end

function CModelTurnManager:getPlayerIndex()
    return self.m_PlayerIndex
end

function CModelTurnManager:isTurnPhaseRequestToBegin()
    return self.m_TurnPhaseCode == TURN_PHASE_CODES.RequestToBegin
end

function CModelTurnManager:isTurnPhaseMain()
    return self.m_TurnPhaseCode == TURN_PHASE_CODES.Main
end

function CModelTurnManager:runTurn()
...
function CModelTurnManager:beginTurnPhaseBeginning(income, repairData, supplyData, callbackOnEnterTurnPhaseMain)
    assert(self:isTurnPhaseRequestToBegin(), "CModelTurnManager:beginTurnPhaseBeginning() invalid turn phase code: " .. self.m_TurnPhaseCode)
...
function CModelTurnManager:endTurnPhaseMain()
    assert(self:isTurnPhaseMain(), "CModelTurnManager:endTurnPhaseMain() invalid turn phase code: " .. self.m_TurnPhaseCode)
